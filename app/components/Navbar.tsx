...
export default function Navbar() {
    ...
    return (
        <header className='sticky top-0 z-50 w-full border-b border-gray-200 dark:border-[#23262d] bg-white dark:bg-[#0F1115] transition-colors duration-300'>
            <nav className='max-w-350 mx-auto px-4 sm:px-6 lg:px-8'>
                <div className='flex items-center justify-between py-2 lg:py-4'>
                    ...
                    <div className='hidden md:flex items-center gap-4'>
                        {navLinks.map((link) => (
                            <button key={link.id} onClick={() => scrollToSection(link.id)} className={`rounded-md px-3 py-2 text-base lg:text-lg tracking-tight transition-all duration-300 cursor-pointer ${activeSection === link.id ? 'text-gray-900 dark:text-white' : 'text-[#626b77] dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-[#1A1D24] hover:text-[#4a515c] dark:hover:text-white'}`}>
                                {link.label}
                            </button>
                        ))}
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out bg-white dark:bg-[#0F1115] ${isOpen ? 'max-h-96 border-t border-gray-100 dark:border-[#23262d]' : 'max-h-0'}`}>

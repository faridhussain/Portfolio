...
export default function Footer() {
    ...
    return (
        <footer className='w-full max-w-350 mx-auto px-4 sm:px-6 lg:px-8 mt-20 lg:mt-28'>
            <div className='border-t border-gray-200 py-6 flex flex-col sm:flex-row items-center justify-between gap-4'>
                <div className='flex items-center gap-5'>
                    {socialLinks.map((social) => (
                        <a key={social.label} href={social.href} target='_blank' rel='noopener noreferrer' aria-label={social.label} className='group relative text-xl text-gray-500 hover:text-gray-800 duration-300'>
                ...
                <p className='text-sm text-[#565d6e] font-light tracking-tight'>&copy; {year}</p>
            </div>
        </footer>
    );
}
